...
def inference_worker(graph, input_q):
    ...
    while True:
        frame = input_q.get()
	t.tic()
	X = cv2.resize(frame/255. - 0.5, dim, interpolation=cv2.INTER_NEAREST).astype(numpy.float16) 
...
def main():
    ...
    pool = Pool(8, inference_worker, (graph, process_q))
    ...
    t=TicToc()
    t.tic()
    vid = cv2.VideoCapture('../data/downsize.mp4')
    while vid.isOpened():
